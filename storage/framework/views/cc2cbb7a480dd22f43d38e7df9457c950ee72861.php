...
<li class="dropdown">
    ...
    <ul id="messages_preview" class="dropdown-menu">
                ...
                <li>
                    <a href="<?php echo e(URL::to("departments/messages")); ?>">Inbox</a>
                </li>
    </ul>
</li>
<?php endif; ?>
<script type="text/javascript">
    getUnread();
    function getUnread(){
        var url = "<?php echo e(URL::to('messages/unread')); ?>";
        var image = "<?php echo e(URL::to('images/img.jpg')); ?>";
        ...
        $.get(url,null,function(response){
            ...
            for(i=0;i<data.length;i++){
                ...
                if(parseInt(message.department_id) != 0){
                    $("#messages_preview").prepend('<li>' +
                            '<a href="<?php echo e(URL::to('departments/conversation')); ?>/'+message.department_id+'/'+message.client_id+'">' +
                            '<span class="message">'+message.message.substr(0,50)+'...</span>' +
                            '</a>' +
                            '</li>');
                }else{
                    $("#messages_preview").prepend('<li>' +
                            '<a href="<?php echo e(URL::to('')); ?>/messages/'+message.id+'">' +
            ...
            for(i=0;i<messages.order.length;i++){
                ...
                if(role=='client'){
                    $("#messages_preview").prepend('<li>' +
                            '<a href="<?php echo e(URL::to('')); ?>/stud/order/'+message.order_id+'#o_messages">' +
                    ...
                    $("#messages_preview").prepend('<li>' +
                            '<a href="<?php echo e(URL::to('')); ?>/order/'+message.order_id+'#o_messages">' +
                            '<span><span>Order #'+message.order_id+'</span></span>' +
                            '<span class="message">'+message.message.substr(0,50)+'...</span>' +
                            '</a>' +
                            '</li>');
...
<?php if(Auth::user()->role=='writer' || Auth::user()->role=='client'): ?>
    <script type="text/javascript">
        var url = '<?php echo e(URL::to('writer/get_count')); ?>';
        $.get(url,null,function(response){
