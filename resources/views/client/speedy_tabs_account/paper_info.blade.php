...
<div id="tab_content_1" class="tab_content_form">
    ...
    <div class="row form-group">
        ...
        <div class="col-xs-12 col-sm-9 col-md-8">
            <textarea rows="3" name="description" type="text"  class="form-control" style="resize: vertical;"></textarea>
        </div>
    </div>
    <div class="row form-group">
        <label  class="col-xs-12 col-sm-3 col-md-4 align-left control-label">Paper details</label>
        ...
        <div class="col-xs-12 col-sm-9 col-md-8">
            <div onchange="getOrderCost();" type="select" class="ButtonGroup btn-group btn-group-justified">
                ...
                @foreach($styles as $style)
                    <a id="style_no_{{ $style->id }}" onclick="changeStyle({{ $style->id }});" href="#" class="btn btn-default style_button {{ $style->id == $style_id ? 'active':'' }}" role="button">{{ $style->label }}</a>
                @endforeach
