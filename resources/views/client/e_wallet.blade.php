        ...
        <div class="panel-body">
            ...
            @include('client.redeem_notice')
                ...
                <div class="row tile_count">
                    ...
                    @if($user->website->author ==1)
                      <div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
                        <span class="count_top"><i class="fa fa-pencil"></i> Articles</span>

                          <div class="count">{{ $user->articles()->count() }}</div>
                          <span class="count_bottom"><i class="green"><i class="fa fa-heart"></i> {{ $user->getUnredeemedArticles() }} </i> Redeemable</span>
<br/>
                      @if(Auth::user()->role == 'admin')
                        <span class="count_bottom"><a href="{{ url("order/articles?tab=published&user=$user->id") }}" class="btn btn-xs btn-info"><i class="fa fa-eye"></i> View Articles</a></span>                        
                        @endif
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            @if($user->getUnredeemedArticles()>300)
                                                <form class="form-horizontal ajax-post" action="{{ URL::to('stud/redeem-articles') }}">
                                                    {{ csrf_field() }}
                                                    <div class="form-group">
                                                        <?php
                                                ...
                                                <label class="control-label col-md-4">Articles</label>
                                                        ...
                                                        <select class="form-control" name="amount">
                                                            @while($i<$selects)
                                                                <?php $i++; ?>
                                                            <option value="{{ $i*50 }}">{{ $i*300 }} articles for ${{ $i*50 }}</option>
                                                            @endwhile
                                                        </select>
                                                        </div>
                                                    </div>
